...
class ProductService {
    async createProduct(data, userId) {
        ...
        if (existingSku) {
            throw new errors_1.ConflictError(`SKU ${data.sku} already exists`);
        }
        // Calculate availability
        const availability = this.calculateAvailability(data.stockQuantity, data.continueSellingOutOfStock || false);
        ...
        const product = await database_1.default.$transaction(async (tx) => {
            ...
            if (data.section === client_1.Section.CAFE) {
                const cafeData = data;
            ...
            else if (data.section === client_1.Section.FLOWERS) {
                const flowersData = data;
            ...
            else if (data.section === client_1.Section.BOOKS) {
    ...
    async getProductById(id) {
        ...
        if (!product) {
            throw new errors_1.NotFoundError('Product', id);
        }
        return this.addSignedUrlsToProduct(product);
    }
    async listProducts(params, userRole, userSection) {
        const page = Number(params.page) || 1;
        const limit = Math.min(Number(params.limit) || env_1.env.DEFAULT_PAGE_SIZE, env_1.env.MAX_PAGE_SIZE);
        ...
        if (params.section === client_1.Section.CAFE) {
            const cafeWhere = {};
            ...
            if (Object.keys(cafeWhere).length > 0) {
                where.cafeProduct = cafeWhere;
            }
        }
        if (params.section === client_1.Section.FLOWERS) {
            const flowersWhere = {};
            ...
            if (Object.keys(flowersWhere).length > 0) {
                where.flowersProduct = flowersWhere;
            }
        }
        if (params.section === client_1.Section.BOOKS) {
            const booksWhere = {};
            if (params.author) {
        ...
        if (params.mostDiscounted) {
            ...
            const topDiscounted = productsWithDiscountCalc
                ...
                .slice(0, 7);
            return {
                products: await this.addSignedUrlsToProducts(topDiscounted),
        ...
        if (cached) {
            const productsWithSignedUrls = await this.addSignedUrlsToProducts(cached.products);
            return {
                ...cached,
        ...
        // Cache for 5 minutes
        await redis_1.cache.set(cacheKey, result, 1);
        return {
            ...result,
            products: await this.addSignedUrlsToProducts(products),
        };
    }
    async updateProduct(id, data, userId) {
        ...
        if (data.sku && data.sku !== existingProduct.sku) {
            const duplicateSku = await database_1.default.product.findUnique({
                where: { sku: data.sku },
            });
            if (duplicateSku) {
                throw new errors_1.ConflictError(`SKU ${data.sku} already exists for another product`);
            }
        }
        ...
        if (data.stockQuantity !== undefined || data.continueSellingOutOfStock !== undefined) {
            availability = this.calculateAvailability(data.stockQuantity ?? existingProduct.stockQuantity, data.continueSellingOutOfStock ?? existingProduct.continueSellingOutOfStock);
        }
        ...
        const product = await database_1.default.$transaction(async (tx) => {
            ...
            if (existingProduct.section === client_1.Section.CAFE && 'cafeAttributes' in data && data.cafeAttributes) {
            ...
            else if (existingProduct.section === client_1.Section.FLOWERS && 'flowersAttributes' in data && data.flowersAttributes) {
            ...
            else if (existingProduct.section === client_1.Section.BOOKS && 'booksAttributes' in data && data.booksAttributes) {
    ...
    async deleteProduct(id, force = false) {
        ...
        if (activeOrders > 0 && !force) {
            throw new errors_1.ConflictError('Cannot delete product with active orders', {
                activeOrders,
            });
        }
        if (force) {
    ...
    async bulkDeleteProducts(productIds, force = false) {
    ...
    calculateAvailability(stockQuantity, continueSellingOutOfStock) {
        if (stockQuantity > 0) {
            return client_1.ProductAvailability.AVAILABLE;
        }
        return continueSellingOutOfStock
            ? client_1.ProductAvailability.AVAILABLE
            : client_1.ProductAvailability.OUT_OF_STOCK;
    }
    async createInventoryItem(product) {
        const productName = product.section === client_1.Section.BOOKS
            ? product.booksProduct?.title
            : product.section === client_1.Section.CAFE
                ? product.cafeProduct?.name
                : product.flowersProduct?.name;
        await database_1.default.inventoryItem.create({
            data: {
    ...
    async invalidateProductCache(section) {
        await redis_1.cache.invalidatePattern('products:list:*');
        if (section) {
            await redis_1.cache.invalidatePattern(`products:${section}:*`);
        }
    }
    async addSignedUrlsToProducts(products) {
        return Promise.all(products.map((product) => this.addSignedUrlsToProduct(product)));
    }
    async addSignedUrlsToProduct(product) {
